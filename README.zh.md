...
> `pi install` 会把包写入你的 settings 并自动加载 `dist/train-agents.ts`（由 `package.json` 的 `pi` 清单声明）。首次运行自动生成 `~/.pi/agent/train-agents/config.json`（默认配置）。

### 安装与管理

| 操作 | 命令 |
|---|---|
| 安装 | `pi install npm:pi-train-agents` |
| 卸载 | `pi remove npm:pi-train-agents` |
| 更新 | `pi update` |
| 固定版本 | `pi install npm:pi-train-agents@0.1.1` |
