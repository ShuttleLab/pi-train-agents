...
> `pi install` adds the package to your settings and auto-loads `dist/train-agents.ts` (declared via the `pi` manifest in `package.json`).
>
> **Uninstall:** `pi remove npm:pi-train-agents` · **Update:** `pi update` · The first run auto-creates `~/.pi/agent/train-agents/config.json` with defaults.
