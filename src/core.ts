...
const ZH: Record<MsgKey, string> = {
  ...
  stNoteAdd: "说明：新增规则(kind=add)依赖同一问题在 ≥minGapEvidence 个会话中重现 + tier2 语义判断 + 逐字 source 溯源闸门。会话内相似指令用字符串相似度去重；跨会话“是否同一问题”由语义层判定，不依赖措辞雷同——中英文皆是如此，措辞差异大时 add 可能产不出，必要时可手写。本工具主要产出 rewrite/remove。",
...
const EN: Record<MsgKey, string> = {
  ...
  stNoteAdd: "Note: new-rule (kind=add) proposals require the same problem to recur across ≥minGapEvidence sessions + tier2 semantic judgment + verbatim source-traceability gates. Similar instructions within a session are deduped by string similarity; whether observations across sessions are the same problem is judged semantically, not by identical wording — for Chinese and English alike, so an add may not be produced when phrasing differs a lot; write it by hand when needed. This tool primarily produces rewrite/remove.",
...
export const MSGS: Record<Lang, Record<MsgKey, string>> = { zh: ZH, en: EN };
...
export function resolveLang(cfg: any): Lang {
...
export const GAP_SIMILARITY_THRESHOLD = 0.5; // only for gapLedger dedup
export const MAX_TRANSCRIPT_TEXT = 4000;
export const MAX_SESSION_CHARS = 120_000;
export const UNIT_TEXT_IN_PROMPT = 160;
...
export function parseSince(since: any): number | null {
...
export const REF_RE = /目录构成|文件结构|技术栈|路由表|部署架构|项目构成|架构细节|数据结构|reference|stack|structure|directory|routing|dependencies|architecture|项目概览|i18n 两种体系|环境变量|框架怪癖|License|许可|技术选型|项目结构|项目组成/i;
...
export function shapeOfArgs(args: any): string {
...
export function isVerbatim(quote: string, corpus: string): boolean {
...
// ── 记忆文件解析 ──────────────────────────────────────────────────────────────
export function parseMemoryUnits(text: string): { units: any[]; tokens: number } {
...
export function clusterGaps(observations: any[]): any[] {
  const clusters: any[] = [];
...
export function riskMax(a: string, b: string): string {
  const rank: any = { low: 0, medium: 1, high: 2 };
  return (rank[b] ?? 0) > (rank[a] ?? 0) ? b : a;
}

export function foldEvidence(records: any[], opts: { minGapEvidence: number; memoryUnits: any[]; gapObservations: any[] }) {
...
// ── Tier2 提案 / 闸门 ─────────────────────────────────────────────────────────
export function effectiveMaxEdits(memoryTokens: number, cfg: any): number {
...
export function applyEdits(text: string, edits: any[]): { text: string; errors: string[] } {
  let cur = text;
  const errors: string[] = [];
...
export function budgetStatus(currentText: string, projectedText: string, cap: number) {
...
export function validateProposal(edits: any[], memText: string, memTokens: number, cfg: any, allQuotes: string[] = [], realSources: Set<string> = new Set()): { ok: boolean; violations: string[] } {
  const v: string[] = [];
...
export function buildAnalysisPrompt(units: any[], distilledText: string, lang: Lang): string {
...
export function buildSynthesisPrompt(fold: any, memoryText: string, budgetTokens: number, realSources: string[] = [], minGapEvidence: number, lang: Lang): string {
...
export async function distillSession(path: string): Promise<{ text: string; userTurns: number; mtime: number; size: number }> {
